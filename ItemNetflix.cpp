...
void ItemNetflix::visualizarLista(vector<ItemNetflix> t_itens)
{
    string delimitador = "----------";

    for (int i = 0; i < (int)t_itens.size(); i++)
    {
        cout << delimitador << endl;
        t_itens[i].visualizarItem();
    }

    cout << delimitador << endl;
}

void ItemNetflix::visualizarLista(vector<ItemNetflix> t_itens, int t_limite)
{
    string delimitador = "----------";
